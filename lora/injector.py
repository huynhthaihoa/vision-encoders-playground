...
def inject_lora_into_model(model, r=8, target_module_name=None, alpha=1, dropout=0., replace=False, verbose=False):#="attention"):
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear) and (target_module_name is None or (target_module_name is not None and target_module_name in name)):
            ...
            lora = LoRALinear(in_f, out_f, r=r, alpha=alpha, dropout=dropout, replace=replace)
